  ...
  <container>
    <div class="search-condition">
      <search :search="search" @searchFunc="searchData">
        <h1 class="search-title" slot="title">人员管理</h1>
      </search>
      <div class="button-groups">
        ...
        <el-button type="warning" plain @click="importWechat">导入微信</el-button>
      </div>
    </div>
    <div class="member-result-box">
      <el-table
        ...
        <el-table-column prop="userid" label="工号" align="center" width="120px"></el-table-column>
        <el-table-column prop="name" label="姓名" align="center"></el-table-column>
        <el-table-column prop="mobile" label="手机" align="center" width="120px"></el-table-column>
        <el-table-column prop="email" label="邮箱" align="center"></el-table-column>
        <el-table-column prop="salt" label="部门" align="center" width="120px"></el-table-column>
        ...
        <el-table-column prop="status" label="状态" align="center" width="120px">
          <template slot-scope="scope">
            <el-tag :type="scope.row.status.code==0?'':'danger'">{{scope.row.status.info}}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog title="资产详细" :visible.sync="dialogModifyInforVisible" center width="55%">
      <div class="dialog-title" slot="title">
        <span v-if="memberDialogStatus==1">添加</span><span v-if="memberDialogStatus==2">修改</span><span
        v-if="memberDialogStatus==3">查看</span>员工信息
    ...
    data() {
      return {
        ...
        search: [
          {
            key: '',
            options: ['IT部', '网络营销部', '市场部', '人事部', '行政部'],
            placeholder: '请选择部门',
            type: 'select'
          },
          {
            key: '',
            options: ['在职', '离职', '待入职', '待岗'],
            placeholder: '请选择在职状态',
            type: 'select'
          },
          {
            key: '',
            options: ['姓名', '部门', '工号', '手机/电话', '邮箱', '企业微信'],
            placeholder: '精确搜索条件',
            type: 'select'
          },
          {
    ...
    methods: {
      ...
      addMember: function (type, mutiData) {
        ...
        if (type === 'add') {
          ...
          this.modifyData.staffID.text = this.tableData[row].userid;
      ...
      beforeAvatarUpload(file) {
        ...
        if (!isJPG) {
          this.errorTip('上传头像图片只能是 JPG 格式!');
        }
        if (!isLt2M) {
          this.errorTip('上传头像图片大小不能超过 2MB!');
        }
        return isJPG && isLt2M;
      },
      importWechat: function () {
      ...
      getDataFunc(page) {
        this.$ajax.get(this.host + '/api/staff', {
          params: {
            page: page,
